...
class DanGa : public Enemy{
    ...
    void KhoiTao(SDL_Renderer* _renderer){
        //Enemy::Run();
        string s;
    x.KhoiTao("./pictures//c1.png",_renderer,2000,3,100,0);
    ...
    void HienThi() {
    for(int i=0;i<dan_ga.size();i++) {
      //  Enemy::Run();
       dan_ga[i].Run();
       dan_ga[i].khung_nv.x += dan_ga[i].speed;
       //dan_ga[i].khung_nv.x = 1-5*cos(i+1);
      // dan_ga[i].khung_nv.x += 3*
       dan_ga[i].khung_nv.y = 100-30*sin(dan_ga[i].khung_nv.x*3.14/180);
       //dan_ga[i].khung_nv.y = cos(dan_ga[i].khung_nv.x/10);
       /// phan tu dong quay tro lai
        dan_ga[i].HienThi();
    }
    }
